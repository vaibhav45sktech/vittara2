...
async function seedDatabase() {
  ...
  try {
    ...
    await client.query('DELETE FROM "Variant"');
    ...
    for (let i = 0; i < products.length; i++) {
      ...
      console.log(`Inserting product ${i + 1}/${products.length}: ${product.name}`);
      
      const result = await client.query(
        'INSERT INTO "Product" (id, name, description, price, image, category, "createdAt") VALUES ($1, $2, $3, $4, $5, $6, NOW()) RETURNING id',
        [require('crypto').randomUUID(), product.name, product.description, product.price, product.image, product.category]
      );
      
      console.log(`Inserted: ${product.name} (ID: ${result.rows[0].id})`);
    }
    
    // Verify count
    const countResult = await client.query('SELECT COUNT(*) FROM "Product"');
    console.log(`Seeding completed. Total products: ${countResult.rows[0].count}`);
